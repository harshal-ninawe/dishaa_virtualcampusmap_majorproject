...
import {
  ...
  Navigation2, Navigation, MapPin, X, CheckCircle2, ArrowRight,
  Building, Coffee, Trophy, Crosshair, Volume2, VolumeX,
} from 'lucide-react';
...
export default function AIChatWindow({
  ...
  const handleUseLiveLocation = (point: 'from' | 'to') => {
    const latest = getLatestLocation();

    const setLiveLoc = (coords: [number, number]) => {
      const liveLoc: CampusLocation = {
        id: `live-location-${Date.now()}`,
        name: '📍 My Live Location',
        type: 'amenity',
        categoryLabel: 'Live GPS',
        description: `Live real-time location (${coords[0].toFixed(5)}, ${coords[1].toFixed(5)})`,
        coords,
      ...
      if (point === 'from') {
        setFromLoc(liveLoc);
        setFromQuery('📍 My Live Location');
      } else {
        setToLoc(liveLoc);
        setToQuery('📍 My Live Location');
      }
    };

    if (latest) {
      setLiveLoc([latest.lat, latest.lng]);
  ...
  const handleGetDirections = () => {
    ...
    if (routes.length > 0) {
      ...
      if (activeRoute.milestones && activeRoute.milestones.length > 0) {
        voiceNavigation.speak(activeRoute.milestones[0].instruction, 'NAVIGATION');
      }
    } else {
          ...
          <div className="shrink-0 px-4 py-3 bg-slate-50/50 dark:bg-zinc-900/50 space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => handleUseLiveLocation('from')}
                  className="px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 text-[10px] font-semibold flex items-center gap-1 hover:bg-emerald-100 transition-colors cursor-pointer"
                  title="Use current live browser GPS location as start point"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => {
                        ...
                        if (directionsState.milestones[prev]) {
                          voiceNavigation.speak(directionsState.milestones[prev].instruction, 'NAVIGATION', undefined, true);
                        }
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (directionsState.milestones[next]) {
                          voiceNavigation.speak(directionsState.milestones[next].instruction, 'NAVIGATION', undefined, true);
                        }
                      }}
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          voiceNavigation.speak(m.instruction, 'NAVIGATION', undefined, true);
                        }}
